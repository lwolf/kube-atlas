...
// RenderReleaseForView renders a single release and returns the YAML content for viewing
func (a *App) RenderReleaseForView(release config.Release, repoRoot string, fsys fs.FS, renderer *render.Engine, patcher *kust.Patcher) (string, error) {
	return a.renderRelease(release, repoRoot, fsys, renderer, patcher)
}
