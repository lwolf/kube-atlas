...
type EditReleaseOptions struct {
	ID      string // Identifies which release to edit
	Chart   string
	Version string
}

// EditRelease updates an existing release's configuration.
...
// EditRelease updates an existing release's configuration.
func (a *App) EditRelease(opts EditReleaseOptions) error {
	cfgPath := "atlas.yaml" // Assume we're in repo root for now
	cfg, err := config.Load(cfgPath)
	if err != nil {
		return fmt.Errorf("failed to load config: %w", err)
	}
	...
	for i, release := range cfg.Releases {
		if release.ID == opts.ID {
			cfg.Releases[i].Chart = opts.Chart
			cfg.Releases[i].Version = opts.Version
			found = true
			break
		}
	}
	...
	if !found {
		return fmt.Errorf("release %q not found", opts.ID)
	}
	...
	fmt.Printf("Updated release %q in atlas.yaml\n", opts.ID)
	return nil
}
